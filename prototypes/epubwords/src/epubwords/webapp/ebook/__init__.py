...
# TODO make all endpoints use templates
...
def upload_file(
    ...
    ebook_extractor: ebook.EbookExtractor = fastapi.Depends(ebook.get_ebook_extractor),
) -> str:
    # TODO extract ebook with ebook extractor
    # TODO add ebook to database
...
def list_ebook_chapters(
    ...
    for c in chapter_records:
        html += f"""
            <tr>
                <td><a href="/{ebook_id}/{c.chapter_number}">{c.start_of_text}...{c.end_of_text}</a></td>
            </tr>
        """
    html += "</tbody></table>"
