...
def list_ebooks(
    ...
    html = "<table>"
    html += f"<thead><tr><td>{add_book_html}</td></tr></thead>"
    html += "<tbody>"
...
def list_ebook_chapters(
    ...
    chapter_records = database_client.get_chapter_list(book_id)
    ...
    for c in chapter_records:
        html += f"""
            <tr>
                <td><a href="/{book_id}/{c.number}">{c.title}</a></td>
            </tr>
        """
    html += "</tbody>"
    html += "</table>"
...
def show_chapter_words(
    ...
    for word in words:
        html += f"""
            <tr>
                <td><a href="/{book_id}/{chapter_number}/{word}">{word}</a></td>
            </tr>
        """
    html += "</tbody>"
    html += "</table>"
